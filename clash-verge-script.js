const BASE_URL = 'https://raw.githubusercontent.com/soizo/proxy-rules/main/rules/';
const PREFERRED_PROXY_GROUPS = ['Proxy', 'PROXY', 'proxy', '节点选择', '🚀 节点选择'];
const PROVIDERS = {
...
function findProxyGroup(config) {
  const groups = Array.isArray(config && config['proxy-groups']) ? config['proxy-groups'] : [];

  for (const name of PREFERRED_PROXY_GROUPS) {
    if (groups.some((group) => group && group.name === name)) return name;
  }

  const selectGroup = groups.find((group) => group && group.type === 'select' && group.name);
...
function main(config, profileName) {
  ...
  const providers = output['rule-providers'] && typeof output['rule-providers'] === 'object' ? output['rule-providers'] : {};
