...
const findProxyGroup = context.module.exports.findProxyGroup || context.findProxyGroup;
...
const config = {
  ...
  'rule-providers': {
    existing: { type: 'http', behavior: 'classical', format: 'text', interval: 3600 },
  },
  'proxy-groups': [
...
assert.deepStrictEqual(plain(output['rule-providers'].existing), config['rule-providers'].existing);
